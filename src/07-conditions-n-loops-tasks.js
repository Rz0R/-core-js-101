...
function doRectanglesOverlap(rect1, rect2) {
  ...
  return !(rectA.y > rectB.y1 || rectB.y > rectA.y1 || rectA.x > rectB.x1 || rectB.x > rectA.x1);
}
...
function findFirstSingleChar(str) {
  ...
  const uniqChars = [...new Set(chArr)];

  for (let i = 0; i < uniqChars.length; i += 1) {
    const uniqCh = uniqChars[i];
    if (uniqCh !== ' ' && chArr.filter((it) => uniqCh === it).length === 1) return uniqCh;
  }
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  const lastNum = a < b ? b : a;
  const startBracket = isStartIncluded ? '[' : '(';
  const endBracket = isEndIncluded ? ']' : ')';

  return `${startBracket}${firstNum}, ${lastNum}${endBracket}`;
}
...
function getDigitalRoot(num) {
  const res = `${num}`.split('').reduce((sum, it) => sum + +it, 0);
